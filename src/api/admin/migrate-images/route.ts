...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  for (const p of products) {
    ...
    const migrateOne = async (rawUrl: string | null | undefined) => {
      if (!rawUrl) return rawUrl
      const url = String(rawUrl).split(";")[0].split(",")[0].trim()
      let host = ""
      try {
      ...
      if (!OLD_HOSTS.includes(host)) return rawUrl
    ...
    try {
      ...
      results.push({ id: p.id, error: String(e?.message || e), warnings })
    }
  }
